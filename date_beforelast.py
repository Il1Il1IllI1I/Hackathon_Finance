...
# 월간 수익률을 계산합니다.
kospi_data['m_return'] = kospi_data['Adj Close'].pct_change(freq='M')

# 필요한 열들에 대해 새로운 DataFrame을 생성합니다.
kospi_df = pd.DataFrame(index=kospi_data.index)
kospi_df['kospi'] = kospi_data['Adj Close']
kospi_df['m_return'] = kospi_data['m_return']
kospi_df['base_date'] = kospi_data.index + BMonthEnd(0)  # 각 날짜에 대해 당월말을 얻습니다.

# 월별로 중복을 제거하고, 마지막 행만 남깁니다. 이렇게 하면 각 월에 대해 한 행만 남게 됩니다.
kospi_df = kospi_df.drop_duplicates(subset='base_date', keep='last')

# 'before_last' - 월말 하루 전 영업일을 계산합니다.
kospi_df['before_last'] = kospi_df['base_date'] - pd.Timedelta(days=1)
...
filename = "date_last_correct.csv"
